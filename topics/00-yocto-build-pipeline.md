...
이 chapter는 Yocto build를 “source를 compile하는 command”가 아니라 metadata를 읽어 task graph를 만들고, 그 결과를 package/rootfs/image/deploy artifact로 이어가는 pipeline으로 보는 관점을 잡는다. 이후 chapter에서 반복해서 등장하는 `layer`, `recipe`, `machine`, `distro`, `image`, `packagegroup`, `bbappend`가 어느 단계에 개입하는지 먼저 정리한다.

## Yocto를 한 문장으로 말하면

Yocto는 Embedded Linux 제품을 만들기 위한 metadata 기반 build system이다.
...
```mermaid
flowchart TB
    Layer["🏛️ Layer<br/>(metadata unit)"] --> LayerConf["📄 conf/layer.conf"]
    Layer --> Recipes["📋 recipes-*/*.bb"]
    Layer --> Appends["📝 *.bbappend"]
    Layer --> Classes["🔧 classes/*.bbclass"]
    Layer --> ConfFiles["⚙️ conf/"]
    Layer --> Files["📂 files/"]

    Recipes --> Inherit["🔗 inherit class"]
    Classes --> Inherit
    Recipes --> Include["🔗 require/include conf"]
    ConfFiles --> Include
    Appends --> Recipes
    Files --> SrcUri["🔗 SRC_URI += file://"]
    SrcUri --> Recipes

    style Layer fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style LayerConf fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Recipes fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Appends fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Classes fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style ConfFiles fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Files fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
```

**Layer composition:**
- `conf/layer.conf`: layer metadata
- `recipes-*/`: software recipes
- `*.bbappend`: recipe extensions
- `classes/`: shared logic
- `conf/`: machine/distro/feature config
- `files/`: patch, config, service files
...
flowchart TB
    ...
    style Configure fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Compile fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Install fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Package fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style QA fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style WritePkg fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Sysroot fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Rootfs fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Image fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Complete fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
    style Deploy fill:#1f2937,stroke:#0e7490,stroke-width:1px,color:#e5e7eb
```

**Phases:**
- Source (Fetch/Unpack/Patch): acquire remote source
- Build (Configure/Compile/Install): compile source
- Package (Package/QA/WritePkg): create packages
- Deploy (Populate/Deploy): final artifacts
...
| variable | 필요한 시점 | 들어가는 곳 | 대표 예 |
| --- | --- | --- | --- |
| `DEPENDS` | compile/link 시점 | recipe sysroot | header, static library, shared library link file |
| `RDEPENDS:${PN}` | target 실행 시점 | target rootfs | 실행에 필요한 package, shared library runtime package, service package |

`DEPENDS`는 “이 recipe를 build하려면 먼저 sysroot에 무엇이 있어야 하는가”를 말한다. compiler가 header를 읽거나 linker가 library를 찾는 순간에 필요하다.

`RDEPENDS`는 “완성된 package가 target에서 실행될 때 무엇이 같이 설치되어야 하는가”를 말한다. rootfs에 설치될 package 관계다.

### Static library 예시

application이 `libfoo.a` 같은 static library를 link하면 library code가 application binary 안에 들어간다.

```bitbake
DEPENDS = "foo-static-library"
```

이 경우 application recipe는 build 시점에 `foo-static-library`의 header와 `libfoo.a`가 필요하므로 `DEPENDS`가 필요하다. 하지만 실행 시점에는 별도 `libfoo.a` package가 target에 설치될 필요가 없다. 그래서 보통 static library 자체를 이유로 `RDEPENDS`를 추가하지 않는다.

### Shared library 예시

application이 `libfoo.so` 같은 shared library를 link하면 build 시점과 실행 시점이 모두 중요하다.

```bitbake
DEPENDS = "foo-shared-library"
RDEPENDS:${PN} += "foo-shared-library"
```

`DEPENDS`는 build 중 header와 `libfoo.so` link file을 sysroot에 넣기 위해 필요하다. `RDEPENDS`는 target에서 application이 실행될 때 dynamic linker가 실제 shared object를 찾을 수 있게 rootfs에 runtime package를 설치하기 위해 필요하다.

Yocto는 shared library packaging metadata를 분석해 runtime dependency를 자동으로 잡아주는 경우가 많다. 그래도 packagegroup, service, script처럼 자동으로 추론되지 않는 관계는 `RDEPENDS`에 명시해야 한다.
...
대표 variable, layer 조작, buildhistory, error debugging, 최종 recipe/variable 확인 방법은 [16. Yocto FAQ와 debugging reference](16-yocto-faq-debugging-reference.md)에서 따로 정리한다.
